...
class Program
{
    static async Task<int> Main(string[] args)
    {
        ...
        {
            await RespondToRpcRequestsAsync(
                        FullDuplexStream.Splice(Console.OpenStandardInput(),
    ...
    {
        await Console.Error.WriteLineAsync($"Connection request #{clientId} received. " +
                                           $"Spinning off an async Task to cater to requests.");
